...
int                     *ft_realloc_values(int *values, size_t size, size_t new_size)
{
    int *values_new_ptr;

    if (size == new_size)
        return (values);
    values_new_ptr = (int*)ft_memalloc(sizeof(int) * new_size);
    if (values)
    {
        ft_memcpy(values_new_ptr, values, size);
        free(values);
    }
    return (values_new_ptr);
}

int                     *ft_extract(int fd)
{
    int     *values;
    char    *str;
    int     i;
    ...
    temp = 0;
    while (i = ft_get_next_line(fd, &str))
    {
        if (i != temp)
            return (-1);
        if (i == 0)
            return (values);
        values = ft_realloc_values(values, size, size + 1);
        temp = i;
    }
}
